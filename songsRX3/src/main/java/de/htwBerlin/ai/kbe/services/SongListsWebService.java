...
public class SongListsWebService {
	...
	public Collection<SongLists> getAllSongLists(@HeaderParam("Authorization") String token,
		...
		System.out.println("//////////7gib public zurueck");
		return songListsDao.findPublicSongById(id);
	...
	// // if (SongLists != null && SongLists.getTitle() != null &&
	// SongLists.getArtist() != null &&
	// // SongLists.getAlbum() != null ) {
	// // System.out.println("createSongLists: Received SongLists: " +
	// SongLists.toString());
	// // return
	// //
	// Response.status(Response.Status.CREATED).entity(SongListsBook.getInstance().addSongLists(SongLists)).build();
	// // }else {
	// // return Response.status(Response.Status.NOT_FOUND).entity("Can't create a
	// this
	// // SongLists bad Payload " ).build();
	// // }
	//
	// if (SongLists != nu-8+897,7ll && SongLists.getTitle() != null &&
	// SongLists.getArtist() != null) {
	// int newId = SongListsDao.saveSongLists(SongLists);
	// return Response.status(Response.Status.CREATED).entity("New SongLists Created
	// ").build();
	// } else {
