...
public class SongWebsService {
	...
	public Response getAllSongs(@HeaderParam("Authorization") String token) {
		...
		if (userIdAuthToken != null) {

			System.out.println("getAllSongs: Returning all Songs!");
			@SuppressWarnings("rawtypes")
			GenericEntity entity = new GenericEntity<Collection<Song>>(SongsStore.getInstance().getAllSongs()) {
			};
			return Response.ok(entity).build();

		} else {
			return Response.status(Response.Status.UNAUTHORIZED).entity("UnAuth").build();
		}
	...
	public Response getSong(@PathParam("id") Integer id, @HeaderParam("Authorization") String token) {
		...
		if (token == null || userIdAuthToken == null) {
			...
			String userId = userAuth.getUserIdByToken(token);
			if (userId == null) {
				return Response.status(Response.Status.UNAUTHORIZED).entity("You don't have any permission").build();

			} else {
				Song song = SongsStore.getInstance().getSong(id);
				if (song != null) {
	...
	public Response delete(@PathParam("id") Integer id, @HeaderParam("Authorization") String token) {
		...
		if (token == null || userIdAuthToken == null) {
			...
			Song deleteID = SongsStore.getInstance().getSong(id);
			if (deleteID != null) {
				SongsStore.getInstance().deleteSong(id);
				return Response.status(Response.Status.NO_CONTENT).entity(": Succes to deleted Song with ID: ").build();

			} else {
				return Response.status(Response.Status.NOT_FOUND)
						.entity(Response.Status.NOT_FOUND + ": Failed to deleted Song with ID: " + id).build();
			}
		}
